<x-app-layout>
<x-slot:header>Edit Berita</x-slot:header>
<x-slot:title>Edit Berita - Speed ID</x-slot:title>

<div class="max-w-3xl mx-auto">
    <form method="POST" action="{{ route('admin.news.update', $post->id) }}" enctype="multipart/form-data" class="bg-surface-alt rounded-xl shadow-card-sm p-6 md:p-8">
        @csrf
        @method('PUT')
                ...
                <div>
                    ...
                    <select id="subdistrict_id" name="subdistrict_id"
                        ...
                        @foreach($subdistricts as $subdistrict)
                            <option value="{{ $subdistrict->id }}" {{ old('subdistrict_id', $post->subdistrict_id) == $subdistrict->id ? 'selected' : '' }}>
                                {{ $subdistrict->name }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div>
                ...
                @if($post->thumbnail)
                    <div class="mt-2 mb-3 flex items-center gap-3">
                        <div class="w-20 h-14 rounded-lg overflow-hidden bg-surface">
                            <img src="{{ asset('storage/' . $post->thumbnail) }}" alt="" class="w-full h-full object-cover">
                        </div>
                        <span class="text-xs text-text-muted">Thumbnail saat ini</span>
                    </div>
                @endif
                ...
                <p class="text-xs text-text-muted mt-1">Format: JPG, PNG, WebP. Maks 2MB. Kosongkan jika tidak ingin mengubah.</p>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div class="flex items-end pb-2">
                    <label class="flex items-center gap-3 cursor-pointer">
                        <input type="checkbox" name="is_emergency" value="1" {{ old('is_emergency', $post->is_emergency) ? 'checked' : '' }}
